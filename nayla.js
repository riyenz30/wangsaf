...
const listMessage = {
text: `Hallo aku Bot Krong (◍•ᴗ•◍)❤, Beliau ini yang menciptakan aku @+62812-8993-5471, dia bukan seorang bot, dia orang yang menciptakan aku, kamu bisa ketik #menu untuk melihat fitur bot (灬º‿º灬)♡`,
...
if (!args[0]) return nayla.sendMessage(from, { text: "[ *DAFTAR* ]\n\nMasukkan Gender", footer: 'Loading...', buttons: [{buttonId: `${prefix + command} x`, buttonText: {displayText: '[💪] Man'}, type: 1}, {buttonId: `${prefix + command} y`, buttonText: {displayText: '[☕] Women'}, type: 1}],headerType: 1 })
if (args[0] == "x") {
daftar.push({id: sender, teman:false, gender: "pria", status:false})
fs.writeFileSync('./db/function/daftar.json', JSON.stringify(daftar))
reply(`[ *YOUR-INFO* ]\n• *Status* : Sukses\n• *Id* : ${sender.split("@")[0]}\n• *User* : Free\n• *Gender* : Pria\n• *Teman* : false\n\n*Note* : Silahkan gunakan fitur #menu untuk melihat apa saja fungsi bot ini:3`)}
if (args[0] == "y") {
...
if (budy == prefix + "sc"){reply("https://facebook.com/RianStwn30")} // JANGAN DI UBAH:(
}} catch (err) {
